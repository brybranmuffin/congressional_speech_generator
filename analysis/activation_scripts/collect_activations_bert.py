...
DEFAULT_BERT_MODEL_DIR = "../../outputs/bert/final"
DEFAULT_DATA_FILE      = "../../data/raw_data/filtered_speeches.csv"
DEFAULT_OUTPUT_DIR     = "../../outputs/activations"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect BERT hidden state activations.")
    parser.add_argument("--model_dir",     type=str,  default=DEFAULT_BERT_MODEL_DIR)
    parser.add_argument("--n_speeches",    type=int,  default=50000)
    parser.add_argument("--layers",        type=str,  default="last",
                        help='"last", "all", or comma-separated indices e.g. "6,12"')
...
def main() -> None:
    ...
    logger.info("Loading BERT from %s", args.model_dir)
    tokenizer = BertTokenizer.from_pretrained(args.model_dir)
    model     = BertModel.from_pretrained(args.model_dir, output_hidden_states=True)
    model.eval()
    model = model.to(device)
    logger.info("Parameters: %d", sum(p.numel() for p in model.parameters()))

    n_hidden_states = model.config.num_hidden_layers + 1  # 13 for bert-base
    ...
    for batch_start in range(0, n_total, args.batch_size):
        ...
        try:
            ...
            for idx in layer_indices:
                hidden = outputs.hidden_states[idx]       # (B, seq, 768)
    ...
    for idx in layer_indices:
        ...
        filename = f"bert_{tag}_{args.n_speeches}.npy"
    ...
    if args.save_metadata:
        meta_path = os.path.join(args.output_dir, f"bert_metadata_{args.n_speeches}.json")
    ...
    print(f"BERT activation verification passed: shape {arr.shape}")
